...
提取方式：
  ...
  D10/D4/D5：三级提取（按优先级递降）
    ┌────────────────────────────────────────────
    │ 第1级：文本层提取（最优，100%靠谱）
    │   检查PyMuPDF提取的文本中是否有D10/D4/D5的行星位置表
    │   JH表格粘贴 → 通常有完整分盘数据
    │   如果有 → 直接解析，标注"文本层提取"
    │
    │ 第2级：用户截屏（次优，准确率极高）
    │   文本层无分盘数据时，主动请求用户提供截屏：
    │   "分盘数据需要从图表中读取。为了确保准确，
    │    请帮我截屏以下3张分盘图（从JHora里直接截屏即可）：
    │    ① D10 (Dasamsha) 盘
    │    ② D4 (Chaturthamsha) 盘
    │    ③ D5 (Panchamsha) 盘
    │    每张截清楚行星缩写和As标记就行。"
    │
    │ 第3级：PDF整页渲染（降级方案，用户不方便截屏时）
    │   用户说"不方便截屏"或要求自动处理时：
    │   ⚠️ 禁止自动crop局部区域！坐标经常偏移导致读错。
    │   正确做法：
    │     1. PyMuPDF渲染整页高清PNG（300DPI）
    │        page.get_pixmap(dpi=300) → pageN_DXX.png
    │     2. 将完整页面图片交给AI，文字提示看哪个盘
    │     3. 提取后立即跑校验12(Ra-Ke)
    │     4. Ra-Ke不通过 → 回退到第2级，请求用户截屏
    └────────────────────────────────────────────
